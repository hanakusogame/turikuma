...
export class Fish extends g.E {
	...
	static escapes: number[] = [1, 1, 1, 1, 1, 50, 10, 10, 10, 50, 50, 80, 80, 80, 80, 80, 80];
	...
	constructor(scene: g.Scene) {
		...
		this.collisionArea = new g.FilledRect({ scene: scene, width: 40, height: 40, cssColor: "#ff000020" });
			...
			if (!this.flg) {
				...
				if (this.num === 2 || this.num === 3) {
					if ((cntLoop % 20) === 0) {
						...
						if ((cntTest % 5) === 0) {
							this.speed = 12;
							if (this.num === 3 && this.muki === -1) {
								this.speed = 17;
							}
						} else {
							if (this.num === 3 && g.game.random.get(0, 2) === 0) {
								turn();
							}
		...
		this.reset = () => {
			...
			if (this.num !== 4) {
				...
				this.y = g.game.random.get(g.game.height - 50, g.game.height);//カニだけ下のほうにしか表示されない
			}
			anchor.scaleX = 1;
			anchor.angle = 0;
